...
const LearningPath = ({ onModuleSelect, completedModules }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 py-8 md:py-20 overflow-x-hidden">
      <div className="max-w-6xl mx-auto px-4">
        
        {/* Header */}
        <div className="text-center mb-8 md:mb-16">
          <div className="flex justify-center mb-4 md:mb-6">
            <BearMascot size="60px" className="md:w-20 md:h-20" />
          </div>
          ...
          <p className="text-base md:text-xl text-gray-600 mb-4 md:mb-8 px-4">
            Master MongoDB through interactive checkpoints
          </p>
          
          {/* Progress Stats */}
          <div className="flex justify-center gap-3 md:gap-8 mb-4 md:mb-8 flex-wrap px-2">
            <div className="bg-white rounded-lg p-3 md:p-4 shadow-md min-w-[90px] md:min-w-0">
              <div className="text-xl md:text-2xl font-bold text-green-600">{completedCheckpoints.length}</div>
              <div className="text-xs md:text-base text-gray-600">Completed</div>
            </div>
            <div className="bg-white rounded-lg p-3 md:p-4 shadow-md min-w-[90px] md:min-w-0">
              <div className="text-xl md:text-2xl font-bold text-blue-600">{currentCheckpoint + 1}</div>
              <div className="text-xs md:text-base text-gray-600">Current Level</div>
            </div>
            <div className="bg-white rounded-lg p-3 md:p-4 shadow-md min-w-[90px] md:min-w-0">
              <div className="text-xl md:text-2xl font-bold text-yellow-600">
                {completedCheckpoints.reduce((total, id) => total + checkpoints[id-1]?.xp || 0, 0)}
              </div>
              <div className="text-xs md:text-base text-gray-600">Total XP</div>
            </div>
          </div>
        </div>
        ...
        <div className="relative overflow-visible">
          {checkpoints.map((checkpoint, index) => (
            <div key={checkpoint.id} className="mb-4 md:mb-8">
              
              {/* Checkpoint */}
              <div className={`flex ${index % 2 === 0 ? 'justify-start' : 'justify-end'} mb-4 px-2 md:px-0`}>
                <div className="relative">
                  ...
                  <button
                    ...
                    className={`
                      w-20 h-20 md:w-32 md:h-32 rounded-full border-2 md:border-4 border-gray-800 flex items-center justify-center
                  ...
                  >
                    ...
                    {getCheckpointStatus(checkpoint.id) === 'locked' && (
                      <div className="absolute inset-0 flex items-center justify-center">
                        <svg className="w-5 h-5 md:w-8 md:h-8 text-gray-600" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    {checkpoint.type === 'start' && (
                      <span className="text-white font-bold text-sm md:text-lg">START</span>
                    )}
                    
                    {checkpoint.type === 'end' && (
                      <span className="text-white font-bold text-sm md:text-lg">END</span>
                    )}
                      ...
                      <div className="text-center text-white relative">
                        {getCheckpointStatus(checkpoint.id) === 'locked' ? (
                          ...
                          <div className="relative flex items-center justify-center">
                            <BearMascot size="60px" className="scale-75 md:scale-100" />
                            <div className="absolute -bottom-1 -right-1 md:-bottom-2 md:-right-2 text-lg md:text-2xl">✏️</div>
                            {getCheckpointStatus(checkpoint.id) === 'completed' && (
                      ...
                      <div className="text-center text-white relative">
                        {getCheckpointStatus(checkpoint.id) === 'locked' ? (
                          ...
                          <div className="relative flex items-center justify-center">
                            ...
                            {getCheckpointStatus(checkpoint.id) === 'completed' && (
                              <div className="absolute -top-1 -right-1 bg-green-500 rounded-full w-5 h-5 md:w-6 md:h-6 flex items-center justify-center text-xs md:text-sm">
                                ✓
                              </div>
                            )}
                          </div>
                  ...
                  <div className={`
                    absolute ${index % 2 === 0 ? 'left-24 md:left-40' : 'right-24 md:right-40'} top-1/2 transform -translate-y-1/2
                    bg-white rounded-lg p-3 md:p-4 shadow-lg border-2 border-gray-200 w-48 md:min-w-64
                    ${getCheckpointStatus(checkpoint.id) === 'locked' ? 'opacity-50' : ''}
                  `}>
                    <h3 className="font-bold text-sm md:text-lg text-gray-800 mb-1 md:mb-2">
                      {checkpoint.title}
                    </h3>
                    ...
                    {getCheckpointStatus(checkpoint.id) !== 'locked' && (
                      <button 
                        onClick={() => handleCheckpointClick(checkpoint.id)}
                        className="w-full mt-2 md:mt-3 bg-blue-600 text-white py-1.5 md:py-2 rounded-lg hover:bg-blue-700 transition-colors text-xs md:text-sm font-medium"
                      >
        ...
        {/* Achievement Section */}
        <div className="text-center mt-8 md:mt-16 bg-white rounded-2xl p-4 md:p-8 shadow-lg">
          <h2 className="text-2xl md:text-3xl font-bold text-gray-800 mb-3 md:mb-4">🏆 Your Progress</h2>
          <div className="w-full bg-gray-200 rounded-full h-3 md:h-4 mb-4 md:mb-6">
            <div 
              className="bg-gradient-to-r from-green-500 to-blue-500 h-3 md:h-4 rounded-full transition-all duration-500"
